...
def load_data(name: str, path: str, supervise: bool) -> List:
    """根据名字加载不同的数据集."""

    def load_snli_data(path, supervise):
        with jsonlines.open(path, "r") as f:
            return [
                (line["origin"], line["entailment"], line["contradiction"])
                if supervise
                else line["origin"]
                for line in f
            ]
    ...
    if name == "snli":
        return load_snli_data(path, supervise)
    elif name == "lqcmc":
